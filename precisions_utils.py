...
def precisions_update_backward_predictions(e, P, lr):
    ...
    with tf.name_scope("PrecisionUpdate"):
        ...
        for i in range(len(P)):
            ...
            P[i].assign_add(tf.scalar_mul(lr, 0.1*I - Peet/b))
...
def precisions_update_forward_predictions(e, P, lr, diagonal=False):
    ...
    with tf.name_scope("PrecisionUpdate"):
        for i in range(len(P)):
            P[i].assign_add(tf.scalar_mul(lr, .1*tf.eye(tf.shape(P[i])[0]) - reduced_batched_outer_product(tf.matmul(P[i], e[i]), e[i])/b))